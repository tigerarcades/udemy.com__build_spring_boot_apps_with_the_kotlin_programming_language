            ...
            for (row in 1..15) {
                for (num in 1..36) {
                    val seatType: SeatType = when {
                        row == 15 -> SEAT_TYPE_BACK_ROW
                        row == 14 -> SEAT_TYPE_CHEAPER_SEAT
                        (row in 1..2) && (num in 4..33) -> SEAT_TYPE_BEST_VIEW
                        (row in 1..13) && num !in 4..33 -> SEAT_TYPE_RESTRICTED
...
class Theater(val seats: List<Seat>) {
...
fun main(args: Array<String>) {
    val cheapSeats = Theater(SeatType.getSeatOrder()).seats.filter { it.price == SeatType.SEAT_TYPE_BACK_ROW.price }
    for (s in cheapSeats) println(s)
}
